...
export class MedicamentoEditComponent {
    ...
    dialogRef.afterClosed().subscribe((result:boolean) => {
      if (result){
        this.medicamentosService.delete(this.Medicamento.Id).subscribe({
          next: () => {
            this.exibirMensagemRedirecionar(`Usuario ${this.Medicamento.Nome} excluído com sucesso`);
          },
          error: (jsonErro: any) => {
            this.exibirMensagemErro(jsonErro.status);
          }            
  ...
  exibirMensagemErro(status: number): void{    
    if (status === 0)
      ...
      this.exibirMensagem('Nenhum medicamento foi encontrado! Alguém excluiu esse medicamento.');
    else if (status === 500)
